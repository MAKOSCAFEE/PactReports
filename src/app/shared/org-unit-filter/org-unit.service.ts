  ...
  getOrgunitLevelsInformation() {
    return Observable.create(observer => {
      if (this.orgUnitLevels.length !== 0) {
        ...
        this.http.get('../../../api/organisationUnitLevels.json?fields=id,name,level&order=level:asc').subscribe(
          (levels: any[]) => {
  ...
  getOrgunitGroups() {
    return Observable.create(observer => {
      if (this.orgUnitGroups.length !== 0) {
        ...
        this.http.get('../../../api/organisationUnitGroups.json?fields=id,name&paging=false').subscribe(
          (groups: any) => {
  ...
  getInitialOrgunitsForTree(orgunits) {
    return Observable.create(observer => {
      if (this.initialOrgUnits !== null) {
        ...
        this.http
          .get(
            '../../../api/organisationUnits.json?fields=id,name,level,children[id,name]&' +
              'filter=id:in:[' +
              orgunits.join(',') +
              ']&paging=false'
          )
