...
export class PeriodService {
  ...
  private _getMonthlyPeriods(year) {
    ...
    const currentPeriod =
      currentDate.getFullYear() + (currentMonth < 10 ? '0' + currentMonth : currentMonth).toString();
    return periods
      .map((period: any) => {
        return {
          ...period,
      ...
      .filter((period: any) => parseInt(period.id, 10) <= parseInt(currentPeriod, 10));
  }
